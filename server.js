...
async function fetchComments() {
  ...
  try {
    ...
    // 🔥 FAST PROCESSING - Sab comments process karo
    for (const msg of res.data.items) {
      if (processedMessages.has(msg.id)) continue;
      processedMessages.add(msg.id);

      // 🔥 AGAR COMMENT WINDOW OPEN HAI TOH PROCESS KARO
      ...
      // 🔥 STEP 3: Agar country mila toh INSTANT game ko bhejo
      if (countryCode) {
        ...
        // 🔥 INSTANT EMIT TO GAME - Sab comments bhejo
        io.emit("newComment", commentData);

        // 🔥 SET TARGET (pehla comment jo aaye)
        if (!currentTarget && canAcceptTarget && !isGameOver) {
          ...
          io.emit("setTarget", { countryCode, username });
        }
    ...
    // 🔥 FAST POLLING - 3 seconds jab window open ho
...
io.on("connection", (socket) => {
  ...
  // 🔥 ROUND START - 30 SECOND TIMER
  socket.on("startRound", () => {
    console.log("🟢 startRound received - 30 second window opening");
    ...
    currentTarget = null;
    canAcceptTarget = true; // 🔥 INSTANT ACCEPT - Pehla comment hi target
    votes = {};
    lastVoter = {};
    roundComments = [];
    processedMessages.clear();

    console.log("🟢 Round STARTED - Comment window OPEN for 30 seconds");

    // 🔥 30 SECOND TIMER
    commentWindowTimer = setTimeout(() => {
      console.log("🔴 30 seconds OVER - Closing comment window");
      isCommentWindowOpen = false;
      canAcceptTarget = false;
      io.emit("commentWindowClosed", { 
        totalComments: roundComments.length,
