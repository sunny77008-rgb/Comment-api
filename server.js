...
function rotateYouTubeKey() {
    ...
    console.log("All YouTube API keys exhausted, resetting...");
...
function startYouTubePolling(liveChatId, apiKey) {
...
app.post("/youtube/start", async (req, res) => {
    ...
    if (!liveChatId && !videoId) return res.status(400).json({ error: "Provide liveChatId or videoId" });
    ...
    if (videoId && !chatId) {
        try {
            ...
            if (data.error) {
                if (data.error.code === 403 || data.error.code === 429) {
                    rotateYouTubeKey();
                    return res.status(429).json({ error: "Quota exceeded, rotated" });
                }
